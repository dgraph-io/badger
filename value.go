...
func (vlog *valueLog) pickLog(head valuePointer, stats map[uint32]int) *logFile {
	vlog.filesLock.RLock()
	defer vlog.filesLock.RUnlock()
	fids := vlog.sortedFids()
	...
	for j := 0; j < i; j++ {
		// TODO acquire mutex before accessing map
		if stats[fids[j]] > candidate.count {
			candidate.fid = fids[j]
			candidate.count = stats[fids[j]]
		}
	}
