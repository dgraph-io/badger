...
func (vlog *valueLog) Open(kv *DB, opt Options) error {
	...
	vlog.gcStats = &gcStats{m: make(map[uint32]int64)}
	return nil
}
...
func (vlog *valueLog) pickLog(head valuePointer) *logFile {
	...
	for j := 0; j < i; j++ {
		fid := fids[j]
		vlog.gcStats.Lock()
		if vlog.gcStats.m[fid] > candidate.discard {
			candidate.fid = fids[j]
			candidate.discard = vlog.gcStats.m[fid]
		}
		vlog.gcStats.Unlock()
	}
...
func (vlog *valueLog) updateGCStats(item *Item) {
	if item.meta&bitValuePointer > 0 {
		...
		vp.Decode(item.vptr)
		vlog.gcStats.Lock()
		vlog.gcStats.m[vp.Fid] += int64(vp.Len)
		vlog.gcStats.Unlock()
	}
}
