...
func TestConcatIterator(t *testing.T) {
	...
	defer tbl.DecrRef()
	tableOpts.Fd = f2
	tableOpts.Mode = options.LoadToRAM
	tbl2, err := OpenTable(tableOpts)
	require.NoError(t, err)
	defer tbl2.DecrRef()
	tableOpts.Fd = f3
	tbl3, err := OpenTable(tableOpts)
	require.NoError(t, err)
	defer tbl3.DecrRef()
...
func TestMergingIterator(t *testing.T) {
	...
	tableOpts := Options{
		Fd:      f1,
		Mode:    options.LoadToRAM,
		ChkMode: options.OnTableAndBlockRead,
	}
	tbl1, err := OpenTable(tableOpts)
	require.NoError(t, err)
	defer tbl1.DecrRef()
	tableOpts.Fd = f2
	tableOpts.Mode = options.LoadToRAM
...
func TestMergingIteratorTakeOne(t *testing.T) {
	...
	t1, err := OpenTable(tableOpts)
	require.NoError(t, err)
	defer t1.DecrRef()
	tableOpts.Fd = f2
	t2, err := OpenTable(tableOpts)
	require.NoError(t, err)
	defer t2.DecrRef()
...
func BenchmarkReadAndBuild(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		func() {
			newBuilder := NewTableBuilder(4 << 10)
			it := tbl.NewIterator(false)
			defer it.Close()
			for it.seekToFirst(); it.Valid(); it.next() {
				vs := it.Value()
