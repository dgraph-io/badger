...
const uidrangeMask uint64 = 0xFFFFFFFFFF000000

func (p ParsedKey) Tags() (res []uint64) {
	atag := farm.Fingerprint64([]byte(p.Attr))
	res = append(res, atag)

	btag := (uint64(p.bytePrefix) << 8) | uint64(p.ByteType)
	if p.Uid > 0 {
		utag := p.Uid & uidrangeMask
		utag |= btag
		res = append(res, utag)
	} else {
		res = append(res, btag)
