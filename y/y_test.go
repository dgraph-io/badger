...
func TestPagebufferReader5(t *testing.T) {
	b := NewPageBuffer(32)
	readerWhenEmptyBuffer := b.NewReaderAt(0)

	readBuffer := []byte{} // Intentionally empty readBuffer.
	n, err := readerWhenEmptyBuffer.Read(readBuffer)
	require.NoError(t, err, "reading into empty buffer should return no error")
	require.Equal(t, 0, n, "read into empty readBuffer should return 0 bytes")
	...
	readerWhenNonEmptyBuffer := b.NewReaderAt(0)
	n, err = readerWhenNonEmptyBuffer.Read(readBuffer)
	require.NoError(t, err, "reading into empty buffer should return no error")
	require.Equal(t, 0, n, "read into empty readBuffer should return 0 bytes")
}
