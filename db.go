...
var (
	...
	lfDiscardStatsKey = []byte("!badger!lfdiscardstats") // For storing lfDiscardStats
)
...
func (db *DB) handleFlushTask(ft flushTask) error {
	if !ft.mt.Empty() {
		...
		discardStatsKey := y.KeyWithTs(lfDiscardStatsKey, db.orc.nextTs())
		ft.mt.Put(discardStatsKey, y.ValueStruct{Value: db.vlog.getEncodedDiscardStats()})
	}
