	...
	for i := 0; i < height; i++ {
		...
		for {
			...
			if prev[i].casNextOffset(i, nextOffset, xOffset) {
				if i == 0 && nextOffset == 0 {
					// Update the bigger offset.
					s.updateBigOffset(x, xOffset)
				}
...
// updateBigOffset will update the given offset key is greater than
// already existing bigger key offsetX in an atomic fashion.
func (s *Skiplist) updateBigOffset(x *node, xOffset uint32) {
	for {
		...
		if atomic.CompareAndSwapUint32(&s.biggestOffset, bOffset, xOffset) {
			// Updated the biggest node offset successfully.
			return
		}
	}
}
