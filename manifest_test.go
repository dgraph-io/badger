...
func TestOverlappingKeyRangeError(t *testing.T) {
	...
	t1, err := table.OpenTable(f, options.MemoryMap, options.OnTableAndBlockRead, true)
	require.NoError(t, err)
	defer t1.DecrRef()
	...
	t2, err := table.OpenTable(f, options.MemoryMap, options.OnTableAndBlockRead, true)
	require.NoError(t, err)
	defer t2.DecrRef()
