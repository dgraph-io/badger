...
- `DB.RunValueLogGC()`: This method is designed to do garbage collection while Badger is online. It
  uses statistics generated by the LSM tree compactions to pick files that are likely to lead to
  maximum space reclamation; if no such statistics are available yet, no file is picked and
  `ErrNoRewrite` is returned. It is recommended to be called during periods of low activity in your
  system, or periodically. One call would only result in removal of at max one log file. As an
  optimization, you could also immediately re-run it whenever it returns nil error (indicating a
  successful value log GC), as shown below.
