...
func (lc *levelsController) AddTable(
	...
	if lc.kv.opt.InMemory {
		if tbl, err = table.OpenInMemoryTable(y.Copy(kv.Value), fileID, &opts); err != nil {
			return fmt.Errorf("while creating in-memory table from buffer: %w", err)
		}
	} else {
		if tbl, err = table.CreateTableFromBuffer(fname, kv.Value, opts); err != nil {
			return fmt.Errorf("while creating table from buffer: %w", err)
		}
	}
	...
	return lc.kv.manifest.addChanges([]*pb.ManifestChange{change}, lc.kv.opt)
}
