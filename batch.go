...
type WriteBatch struct {
	...
	throttle *y.Throttle
	err      error
	flushed  int32 // flushed is used to avoid calling throttle.Finish twice
}
...
func (wb *WriteBatch) Cancel() {
	// check if wb.throttle.Finsh() has been called already, if yes return.
	if atomic.AddInt32(&wb.flushed, 1) > 1 {
		atomic.AddInt32(&wb.flushed, -1)
		return
	}
...
func (wb *WriteBatch) Flush() error {
	...
	// check if wb.throttle.Finish() has been already, if yes return.
	if atomic.AddInt32(&wb.flushed, 1) > 1 {
		atomic.AddInt32(&wb.flushed, -1)
		return wb.err
	}
