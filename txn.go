...
type oracle struct {
	...
	// committedTxns contains all committed writes (contains fingerprints of keys written and their latest commit counter).
...
func (o *oracle) newCommitTs(txn *Txn) uint64 {
	...
	y.AssertTruef(ts > o.lastCleanupTs, "ts: %d should not be less than lastCleanupTs: %d", ts, o.lastCleanupTs)
...
func (o *oracle) cleanupCommittedTransactions() { // Must be called under o.Lock
	...
	y.AssertTruef(maxReadTs >= o.lastCleanupTs, "maxReadTs: %d should not be less than lastCleanupTs: %d", maxReadTs, o.lastCleanupTs)
