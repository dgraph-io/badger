...
func TestBackup(t *testing.T) {
	...
	db1, err := Open(filepath.Join(tmpdir, "backup0"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBackupRestore3(t *testing.T) {
	...
	// backup
	{
		db1, err := Open(filepath.Join(tmpdir, "backup1"))
		if err != nil {
			t.Fatal(err)
		}
	...
	// restore
	db2, err := Open(filepath.Join(tmpdir, "restore1"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBackupLoadIncremental(t *testing.T) {
	...
	// backup
	{
		db1, err := Open(filepath.Join(tmpdir, "backup2"))
		if err != nil {
			t.Fatal(err)
		}
	...
	// restore
	db2, err := Open(filepath.Join(tmpdir, "restore2"))
	if err != nil {
		t.Fatal(err)
	}
	require.NoError(t, db2.Load(&bb, 16))
