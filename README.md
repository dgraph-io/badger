 ...
 * [Getting Started](#getting-started)
    ...
    * [Associative Merge Operations](#associative-merge-operations)
...
Badger provides support for unordered associative merge operations. You can
define a func of type `MergeFunc` which takes in an existing value, and a value
to be _merged_ with it. It returns a new value which is the result of the
_merge_ operation. All values are specified in byte arrays. For e.g., here is a
merge function (`add`) which adds a `uint64` value to an existing `uint64`
value.
...
`MergeOperator.Get()` method can be used to retrieve the latest value of the key
